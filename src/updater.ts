...
async function explainLinuxLimitOrError(
  dialog: typeof import("@tauri-apps/plugin-dialog"),
  err: unknown,
  productName: string,
): Promise<void> {
  const msg = String(err).toLowerCase();
  const looksLikeLinuxLimit =
  ...
  if (looksLikeLinuxLimit) {
    await dialog.message(
      `${productName} was installed via your system package manager. ` +
        `To upgrade, run:\n\n  sudo apt install --only-upgrade <package>\n\n` +
        `In-app updates are available only when running the AppImage build.`,
