...
export function installMenuBar(container: HTMLElement, menus: MenuDef[]): void {
  ...
  document.addEventListener("mousedown", (e) => {
    ...
    if (target && (open.dropdown.contains(target) || open.trigger.contains(target))) return;
    closeOpen();
  });
 ...
 *  one version surface. */
export function installHamburgerMenu(
  button: HTMLElement,
  menus: MenuDef[],
  aboutLine?: string,
): void {
  const merged: MenuDef = { label: "", items: [] };
  if (aboutLine) merged.items.push({ label: aboutLine, static: true });
  menus.forEach((menu) => {
    if (merged.items.length > 0) merged.items.push({ sep: true });
