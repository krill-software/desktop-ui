...
- **`lucideIcon(node, size?)`** — a [Lucide](https://lucide.dev) glyph as an inline SVG for chrome (sidebar rows, toolbar buttons, list icons): `currentColor` stroke, 16px default, `aria-hidden`. Takes the icon node (`import { Folder } from "lucide"`) so bundles tree-shake. Imported from the `@krill-software/desktop-ui/lucide` subpath, not the index, so apps that don't draw glyphs never pull in `lucide` — it's an optional peer dep.
