...
export function buildTitlebar() {
    ...
    const titlebar = tpl.content.firstElementChild;
    const title = titlebar.querySelector("#titlebar-title");
    const menuBar = titlebar.querySelector("#menu-bar");
    ...
    bind(titlebar.querySelector("#titlebar-max"), async () => (await w.isMaximized()) ? w.unmaximize() : w.maximize());
    bind(titlebar.querySelector("#titlebar-close"), () => w.close());
    titlebar.querySelector("#titlebar-drag")?.addEventListener("dblclick", async () => (await w.isMaximized()) ? w.unmaximize() : w.maximize());
